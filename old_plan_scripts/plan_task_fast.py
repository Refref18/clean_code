...
# ── summary ───────────────────────────────────────────────────────────────────
def _print_summary_task(df_r, task, goal_mode, min_n, max_n, simulate,
                        t_total, t_plan_total, t_sim_total, cfg):
    W = 108
    print(f"\n{'='*W}")
    print(f"  SUMMARY  task={task}  goal={goal_mode}  N={min_n}..{max_n}  simulate={simulate}")
    print(f"{'='*W}")

    def _pct(num, den): return f"{100*num/den:.1f}%" if den else "—"

    if simulate:
        hdr = (f"{'N':>2}  {'total':>5}  {'no_plan':>7}  {'no_plan%':>8}  "
               f"{'success':>7}  {'success%':>8}  {'partial':>7}  {'partial%':>8}  "
               f"{'fail':>4}  {'fail%':>5}  {'collapse':>8}  {'collapse%':>9}  "
               f"{'avg_pct':>7}  {'t_plan_avg':>10}  {'t_sim_avg':>9}")
    else:
        hdr = (f"{'N':>2}  {'total':>5}  {'no_plan':>7}  {'no_plan%':>8}  "
               f"{'verified':>8}  {'verified%':>9}  {'unverified':>10}  {'unverif%':>8}  "
               f"{'timeout':>7}  {'timeout%':>8}  {'t_plan_avg':>10}")
    print(hdr)
    print("-" * W)

    totals = dict(total=0, no_plan=0, success=0, partial=0, fail=0, collapse=0,
                  verified=0, unverified=0, timeout=0,
                  t_plan=0.0, t_sim=0.0, pct_sum=0.0, pct_n=0)
    ...
    for n in range(min_n, max_n + 1):
        ...
        t_pa    = sub["t_plan_sec"].mean() if "t_plan_sec" in sub else 0.0
        totals["total"]   += tot
        totals["no_plan"] += int(no_plan)
        totals["timeout"] += int(timeout)
        totals["t_plan"]  += sub["t_plan_sec"].sum() if "t_plan_sec" in sub else 0.0
        ...
        if simulate:
            ...
            t_sa     = sub["t_sim_sec"].mean() if "t_sim_sec" in sub else 0.0
            totals["success"]  += int(success)
            totals["partial"]  += int(partial)
            totals["fail"]     += int(fail)
            totals["collapse"] += int(collapse)
            totals["t_sim"]    += sub["t_sim_sec"].sum() if "t_sim_sec" in sub else 0.0
            if len(ran):
                totals["pct_sum"] += ran["pct"].sum()
                totals["pct_n"]   += len(ran)
            print(f"{n:>2}  {tot:>5}  {no_plan:>7}  {_pct(no_plan,tot):>8}  "
                  f"{success:>7}  {_pct(success,tot):>8}  "
                  f"{partial:>7}  {_pct(partial,tot):>8}  "
                  f"{fail:>4}  {_pct(fail,tot):>5}  "
                  f"{collapse:>8}  {_pct(collapse,tot):>9}  "
    ...
    if simulate:
        ...
        avg_pct = f"{totals['pct_sum']/totals['pct_n']:.1f}%" if totals["pct_n"] else "—"
        print(f"{'':>2}  {T:>5}  {totals['no_plan']:>7}  {_pct(totals['no_plan'],T):>8}  "
              f"{totals['success']:>7}  {_pct(totals['success'],T):>8}  "
              f"{totals['partial']:>7}  {_pct(totals['partial'],T):>8}  "
        ...
        t_pa = totals["t_plan"] / max(T, 1)
        print(f"{'':>2}  {T:>5}  {totals['no_plan']:>7}  {_pct(totals['no_plan'],T):>8}  "
              f"{totals['verified']:>8}  {_pct(totals['verified'],T):>9}  "
              f"{totals['unverified']:>10}  {_pct(totals['unverified'],T):>8}  "
...
def main():
    ...
    _print_summary_task(df_r, task, goal_mode, min_n, max_n, simulate,
                        t_total, t_plan_total, t_sim_total, cfg)
