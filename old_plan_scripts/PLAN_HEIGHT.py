...
def write_no_plan_diagnosis(cfg, group_idx, target_height,
                             group_label, pair_order,
                             obj_symbols_for_group,
                             collapse_syms, rel_dim, obj_dim,
                             planner_output):
    """
    Writes a human-readable diagnosis txt for a NO_PLAN case.
    Checks:
      1. Which object symbols were assigned and whether each bit-pattern
         appears in any operator precondition in domain.pddl.
      2. Whether the target height H{target_height} predicate exists in
         the domain at all.
      3. Raw planner output tail for any error messages.
    Saved to: <pddl_dir>/no_plan_files/group_{idx}_H{h}_diagnosis.txt
    """
    dst_dir = os.path.join(cfg["_pddl_dir"], "no_plan_files")
    os.makedirs(dst_dir, exist_ok=True)
    out_path = os.path.join(dst_dir, f"group_{group_idx}_H{target_height}_diagnosis.txt")

    lines = []
    lines.append(f"NO_PLAN DIAGNOSIS")
    lines.append(f"=================")
    lines.append(f"Group     : {group_label}")
    lines.append(f"Group idx : {group_idx}")
    lines.append(f"Target H  : {target_height}")
    lines.append(f"rel_dim   : {rel_dim}   obj_dim: {obj_dim}")
    lines.append(f"collapse_syms: {collapse_syms}")
    lines.append("")

    # ── 1. Object symbols assigned ───────────────────────────────────────────
    lines.append("── Object symbols ──────────────────────────────────────────")
    for k, (key, sym) in enumerate(zip(pair_order, obj_symbols_for_group)):
        lines.append(f"  obj{k}  {key:40s}  →  {sym}")
    lines.append("")

    # ── 2. Load domain.pddl and check coverage ───────────────────────────────
    domain_path = cfg["_domain"]
    if not os.path.exists(domain_path):
        lines.append(f"[ERROR] domain.pddl not found at {domain_path}")
    else:
        with open(domain_path) as f:
            domain_text = f.read()

        # Check H{target_height} predicate
        h_pred = f"(H{target_height})"
        if h_pred in domain_text:
            lines.append(f"[OK]   {h_pred} found in domain.pddl")
        else:
            lines.append(f"[MISS] {h_pred} NOT found in domain.pddl — domain only goes up to a lower height")
        lines.append("")

        # Check each object symbol bit-pattern against domain operator preconditions
        lines.append("── Object symbol coverage in domain operators ──────────────")
        for k, sym in enumerate(obj_symbols_for_group):
            # Build the PDDL predicate fragment that would appear in a precondition
            # for this object: (z0 objX) or (not_z0 objX) etc.
            # We check if ANY operator references this exact bit pattern for ANY object name.
            bit_predicates = []
            for j, val in enumerate(sym):
                pred = f"(z{j} " if val == 1 else f"(not_z{j} "
                bit_predicates.append(pred)

            # A symbol is "covered" if ALL its bit predicates appear somewhere in the domain
            covered = all(p in domain_text for p in bit_predicates)
            status  = "[OK]  " if covered else "[MISS]"
            lines.append(f"  {status} obj{k} sym={sym}  ({pair_order[k]})")
            if not covered:
                missing_bits = [p for p in bit_predicates if p not in domain_text]
                lines.append(f"         missing predicates: {missing_bits}")
        lines.append("")

        # Check each object symbol appears in an action precondition specifically
        # (not just defined in :predicates) — look for z{j} inside (:action blocks
        lines.append("── Bit predicates present in action preconditions ──────────")
        import re as _re
        action_blocks = _re.findall(r'\(:action.*?(?=\(:action|\Z)', domain_text, _re.DOTALL)
        lines.append(f"  Total actions in domain: {len(action_blocks)}")
        for k, sym in enumerate(obj_symbols_for_group):
            matching_actions = []
            for j, val in enumerate(sym):
                pred = f"(z{j} " if val == 1 else f"(not_z{j} "
                for act in action_blocks:
                    act_name_match = _re.match(r'\(:action\s+(\S+)', act)
                    act_name = act_name_match.group(1) if act_name_match else "?"
                    if pred in act and act_name not in matching_actions:
                        matching_actions.append(act_name)
            lines.append(f"  obj{k} sym={sym}: {len(matching_actions)} actions reference its bits")
            if not matching_actions:
                lines.append(f"         → NO operator uses this symbol pattern — planner cannot place obj{k}")
        lines.append("")

    # ── 3. problem.pddl snapshot ──────────────────────────────────────────────
    problem_path = os.path.join(cfg["_pddl_dir"], "problem.pddl")
    if os.path.exists(problem_path):
        with open(problem_path) as f:
            problem_text = f.read()
        lines.append("── problem.pddl ─────────────────────────────────────────────")
        lines.append(problem_text)
        lines.append("")

    # ── 4. Planner output tail ────────────────────────────────────────────────
    lines.append("── Planner output (last 40 lines) ──────────────────────────")
    planner_tail = "\n".join(planner_output.strip().splitlines()[-40:])
    lines.append(planner_tail)
...
def run_simulation(plan_order_result, pair_to_original, cfg, group_idx, target_height):
    ...
    # Disconnect ALL open PyBullet clients before every simulation.
    # env.py's load_object calls p.loadURDF / p.getAABB with no physicsClientId,
    # so they always target client 0. If a previous simulation left client 0
    # open, the next p.connect() gets client 1, 2, etc. — but loadURDF still
    # goes to client 0 (the stale one) and loads URDF bodies with no mesh
    # geometry, causing getAABB to fail. Disconnecting everything first
    # guarantees p.connect() always returns 0.
